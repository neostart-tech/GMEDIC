            ...
            <div class="checkout-section-content">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" id="customerFirstName" placeholder="Votre prénom"
                            required>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="customerCompany"
                        placeholder="Nom de l'hôpital ou clinique" required>
                </div>

                <div class="form-group">
                    <label class="form-label">Adresse *</label>
                    <input type="text" class="form-input" id="customerAddress" placeholder="Adresse complète"
                        required>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" id="customerZipCode" placeholder="Code postal"
                            required>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="tel" class="form-input" id="customerPhone" placeholder="Numéro de téléphone"
                        required>
                </div>

                <div class="form-group">
                    <label class="form-label">Notes de livraison (optionnel)</label>
                    <textarea class="form-input" id="deliveryNotes" placeholder="Instructions spéciales pour la livraison..."
                        rows="3"></textarea>
                </div>
  ...
  <script>
    ...
    async function refreshCartData() {
        try {
            ...
            displayCartItems(); // Cette fonction met à jour l'affichage
        } catch (error) {
    ...
    async function processPayment() {
        ...
        try {
            ...
            document.getElementById('finalTotalAmount').textContent = total.toLocaleString('fr-FR', {minimumFractionDigits: 2}) + ' fcfa';
    ...
    function updateCartSummary() {
        ...
        document.getElementById('shippingAmount').textContent = shipping.toLocaleString('fr-FR', {minimumFractionDigits: 2}) + ' fcfa';
...
function debugClick(rowId, change) {

    console.log('Type de rowId:', typeof rowId);
    
    // Vérifier si l'item existe
    const item = cartItems.find(item => {
        console.log('Item rowId:', item.rowId, 'Type:', typeof item.rowId);
...
// Version corrigée de changeQuantity
async function changeQuantity(rowId, change) {
    
    // Nettoyer le rowId au cas où
    const cleanRowId = String(rowId).trim();
    
    try {
        // Rechercher l'item avec différentes méthodes
        ...
        if (!item) {
            ...
            console.log('CartItems disponibles:', cartItems.map(i => ({ rowId: i.rowId, name: i.name })));
        ...
        // Rafraîchir les données du panier depuis le serveur
    ...
    async function updateQuantityDirect(rowId, newQuantity) {
        try {
            ...
            closeLoading();
            showError('Erreur lors de la mise à jour de la quantité');
            // Forcer le rafraîchissement en cas d'erreur
            await refreshCartData();
        }
    }
    ...
    function validateQuantityInput(rowId, input) {
        ...
        if (isNaN(quantity) || quantity < 1) {
            ...
            // Mettre à jour immédiatement si valide
